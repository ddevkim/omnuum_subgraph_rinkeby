...
import { Airdrop, PublicMint, SetPublicSchedule } from '../types/OmnuumMintManager/OmnuumMintManager';
...
export function handleAirdrop(event: Airdrop): void {
  ...
  const transaction = saveTransaction(event, getEventName(EventName.Airdrop));

  // no mint schedule, max quantity, no price for airdrop
  mintEntity.block_number = event.block.number;
  mintEntity.mint_transaction = transaction.id;
  mintEntity.topic = getMintTopic(MintTopic.AIRDROP);
  mintEntity.nft_contract = event.params.nftContract.toHexString();
  mintEntity.minter = event.params.receiver;
  mintEntity.mint_quantity = event.params.quantity;
  mintEntity.save();
}
